...
export default function LoadingIndicator({
    ...
    return (
        <div className={`inline-flex items-center gap-2 ${className ?? ''}`} role='status' aria-live='polite'>
            <span className={`animate-spin rounded-full ${SIZE_CLASSES[size]} ${spinnerClasses}`} aria-hidden='true' />
            {label && <span className={labelClassName ?? 'theme-label text-sm font-medium'}>{label}</span>}
        </div>
    );
}
